package com.yin.picture.constant;
...
public class RedisLuaScriptConstant {
    ...
    public static final RedisScript<Long> THUMB_SCRIPT = new DefaultRedisScript<>("""  
            local pictureExistsKey = KEYS[1]
            local tempThumbKey = KEYS[2]
            local userThumbKey = KEYS[3]
            local userId = ARGV[1]             -- 用户 ID
            local pictureId = ARGV[2]             -- 博客 ID
            -- 1. 检查博客是否存在
            if redis.call('SISMEMBER', pictureExistsKey, pictureId) == 0 then
               return -2  -- 返回 -2 表示博客不存在
            end
