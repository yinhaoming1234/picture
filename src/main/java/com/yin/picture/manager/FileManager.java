...
// 封装原图返回结果
            return buildResult(originFilename,multipartFile.getSize(),  imageInfo,uploadPath);
        } catch (Exception e) {
    ...
    private UploadPictureResult buildResult(String originFilename, Long picSize,ImageInfo imageInfo,String uploadPath) {
        // 封装返回结果
        UploadPictureResult uploadPictureResult = new UploadPictureResult();
    ...
    public UploadPictureResult uploadPictureByUrl(String fileUrl, String uploadPathPrefix) {
        ...
        try(InputStream inputStream = response.bodyStream()) {
            ObjectMetadata objectMetadata = new ObjectMetadata();
            objectMetadata.setContentLength(response.contentLength());
            // 上传图片
            PutObjectResult putObjectResult = cosManager.putPictureObject(uploadPath, inputStream, objectMetadata);
            ImageInfo imageInfo = putObjectResult.getCiUploadResult().getOriginalInfo().getImageInfo();
            ProcessResults processResults = putObjectResult.getCiUploadResult().getProcessResults();
...
// 封装原图返回结果
            return buildResult(originFilename,response.contentLength(),  imageInfo,uploadPath);
        } catch (Exception e) {
            log.error("图片上传到对象存储失败", e);
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "上传失败");
        }
